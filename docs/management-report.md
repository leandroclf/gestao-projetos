...
O envio usa `GCHAT_GERENCIAL_WEBHOOK_URL` e a thread `gestao-gerencial` por padrão. O webhook operacional (`GCHAT_WEBHOOK_URL`) não é usado pelo relatório. A publicação gerencial inclui uma visão diária com registros sem alertas, andamento por frente, concluídos/avanços, próximos passos, backlog e foco inferido do ciclo, além dos indicadores e até três exceções críticas. O relatório completo permanece disponível no terminal e no Notion. Como o card repete o texto com markup e botões, cada bloco usa um limite conservador de 8.000 caracteres para evitar rejeição por tamanho após o processamento do Google Chat. As mensagens usam cards `cardsV2` com o nome do projeto, logo opcional, formatação visual por categoria e botões para abrir os links; o campo `text` permanece como fallback.

## Visão gerencial diária

A seção `Visão gerencial diária` é derivada somente dos registros já coletados no Notion. Um item só entra como “sem pendências críticas ou alertas” quando não possui achado habilitado no ciclo. Projetos e tarefas são agrupados pela `Área` ou pela área do projeto relacionado; quando a frente não estiver disponível, o relatório informa `Frente não identificada`.

A seção também lista atividades concluídas ou atualizadas, próximos passos a partir do comentário mais recente, itens de backlog (`Backlog`, `Inbox`, `Ready` e equivalentes) e prioridades. Como o modelo atual não possui uma propriedade explícita de sprint ou objetivo, o foco do ciclo e o objetivo da frente são identificados como inferências baseadas em status, prioridade, prazo, projeto e comentários. O relatório informa explicitamente quando não há sprint oficial registrada; não cria objetivos ou dados de planejamento.

São consideradas críticas as ocorrências de bloqueio, prazo vencido, aprovação pendente, aprovador ausente ou solicitação P0. O prazo definido para o dia corrente permanece válido até 23h59; o achado `overdue` só aparece a partir do dia seguinte, usando o fuso `America/Sao_Paulo`. Pendências de qualidade, como ausência de prazo ou template incompleto, não aparecem individualmente no canal gerencial; permanecem no relatório detalhado e nos alertas operacionais aplicáveis.
